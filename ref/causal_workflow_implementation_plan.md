...
| 2026-07-25 | **Math bridge Levels 2–3 done** — Bayesian networks (d-separation explorer, empirical Markov checks incl. Berkson's bias, mini-PC skeleton 14/14 + spurious NC edges → FCI motivation, Markov equivalence limit) and the causal step (P(Y\|T) vs P(Y\|do(T)), g-computation from scratch recovering truth, front-door criterion-violation demo, rung-3 counterfactuals on the SCM) | `notebooks/math_bridge/level2_bayesian_networks.py`, `level3_causal_step.py` |
| 2026-07-25 | **P4 done** — event-driven UCL runner (`loops/event_runner.py`): batch events → monitor → drift alarm → autonomous actuator re-run; demo stream recovers holiday truth with zero human steps; 37/37 tests green across the pyramid | `runs/event_loop_log.json` |
| 2026-07-25 | **Math bridge Levels 4–5 done** — estimation theory (three estimators of one functional; double robustness surviving one wrong nuisance; the nD trap: Lasso plug-in mean bias 0.39 vs DML 0.04) and the compositional capstone (do() as wire surgery in an exact tensor engine; back-door ≡ surgery to 1e-10) | `notebooks/math_bridge/level4_estimation_theory.py`, `level5_compositional_capstone.py` |
| 2026-07-25 | **Tier-1 gallery started** — Berkeley Simpson's paradox and LaLonde/NSW reproductions (naive −$15,205 → PS matching +$2,697 vs $1,794 RCT benchmark; overlap alarm as pivot) | `notebooks/tier1_gallery/` |

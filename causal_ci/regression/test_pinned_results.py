...
def test_ground_truth_pins(regime, pin):
    ...
    for key, val in pin.items():
        assert abs(truth[key] - val) < 0.005, f"{regime.name}.{key}: {truth[key]} vs pinned {val}"


def test_static_pass_estimate_pin():
    report, _ = run_pass(regime="static", n=20_000, seed=0)
    assert abs(report.estimate.estimate - PIN_STATIC_PASS_ATE) < 0.005
...
def test_aipw_estimator_pin():
    ...
    assert abs(res["ate"] - PIN_STATIC["ate"]) < 0.02  # within estimator noise
    assert 0 < res["se"] < 0.01

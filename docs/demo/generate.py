    ...
    "stations": [
        {
            ...
            "thinking": {
                ...
                "steps": [
                    ...
                    {"label": "Conclusion", "detail": "The target trial emulation framework gives us a precise estimand. <b>First the estimand, then the method — never the reverse</b> (Hern\án & Robins 2016)."}
                ],
                ...
                "refs": "Hern\án & Robins (2016) \“Using Big Data to Emulate a Target Trial\” <i>Am J Epi</i>; Hern\án & Robins (2020) <i>Causal Inference: What If</i> Ch.1-3; Pearl (2009) <i>Causality</i> Ch.1; Imbens & Rubin (2015) <i>Causal Inference</i> Ch.1-2"
            },
        },
        {
        ...
        },
        {
            ...
            "thinking": {
                ...
                "refs": "Hern\án & Robins (2020) <i>Causal Inference: What If</i> Ch.4; Petersen et al. (2012) \“Diagnosing and responding to violations in the positivity assumption\” <i>Epidemiology</i>; Rubin (2008) \“For objective causal inference, design trumps analysis\”; Imbens & Rubin (2015) Ch.13-14"
            },
        },
        {
            ...
            "thinking": {
                ...
                "refs": "Pearl (2009) \§3.3 M-bias; Hern\án, Hern\ández-D\íaz & Robins (2004) \“A structural approach to selection bias\”; Cinelli, Forney & Pearl (2022) \“A Crash Course in Good and Bad Controls\”; VanderWeele & Shpitser (2011) \“A new criterion for confounder selection\”; Pearl (2011) bias amplification"
            },
        },
        {
            ...
            "thinking": {
                ...
                "derivation": {"title": "Why the AIPW score is Neyman-orthogonal", "body": "The naive plug-in estimator is:<br>\τ\̂_plug-in = (1/n)\∑_i [\μ\̂_1(X_i) - \μ\̂_0(X_i)]<br><br>This depends on \μ\̂ being accurate. If \μ\̂ is biased (as all ML models are), the bias propagates directly.<br><br>The AIPW score adds correction terms:<br>\ψ_i = (\μ\̂_1 - \μ\̂_0) + T_i(Y_i - \μ\̂_1)/\ê_i - (1-T_i)(Y_i - \μ\̂_0)/(1-\ê_i)<br><br>The corrections are <b>inverse-probability-weighted residuals.</b> Their expectation is zero when either \μ\̂ or \ê is correct. More deeply:<br>\∂/\∂\η E[\ψ(T,Y,X; \θ, \η)]|_{{\η}={\η}_0} = 0<br>where \η = (\μ, e) are the nuisance parameters. This is the definition of Neyman orthogonality (Chernozhukov et al. 2018, Def. 2.1)."},
                "logic": [
                    ...
                    {"if": "Outcome model \μ\̂_t(X) is correctly specified", "then": "G-computation alone gives a consistent estimate of ATE", "holds": True, "note": "Standard regression adjustment"},
                    {"if": "At least one of \ê or \μ\̂ is correct", "then": "AIPW is consistent (double robustness)", "holds": True, "note": "Even if both are slightly misspecified, the orthogonal score reduces bias"},
                ...
                "pitfalls": [
                    {"mistake": "\“Use ML predictions as a plug-in: ATE = mean(\μ\̂_1 - \μ\̂_0)\”", "consequence": "The ML\’s regularization bias (shrinkage) propagates directly into the causal estimate. In nD settings, the bias can be as large as the effect itself.", "why": "ML minimizes prediction error, not causal error. The orthogonal score debiases ML predictions for causal use."},
        ...
        },
        {
            ...
            "thinking": {
                ...
                "steps": [
                    ...
                    {"label": "Mental operation 2: Run the random common cause test", "detail": "Add a random N(0,1) variable to the adjustment set and re-estimate. A spurious variable should not change the estimate. <b>Our result: \\u0394\τ\̂ = 7.3\×10\\u207b\\u2075</b> (effectively zero). <b>PASS \✓.</b> The estimate is not sensitive to irrelevant covariates."},
                    {"label": "Mental operation 3: Run the subset refuter", "detail": "Re-estimate on a random 80% subset. The estimate should be stable. <b>Our result: \\u0394\τ\̂ = 0.0052</b> (within tolerance of 0.019). <b>PASS \✓.</b> The estimate is not driven by a small subset of the data."},
                    {"label": "Mental operation 4: Run the negative control test", "detail": "NC (battery drain) shares the confounder U with Y but has NO causal effect from T. Estimate T\→NC: if our adjustment is correct, this should be ~0. <b>Our result: \τ\̂(T\→NC) = -0.0008</b> (within tolerance of 0.015). <b>PASS \✓.</b> No evidence of residual confounding."},
                ...
                "logic": [
                    {"if": "Placebo test fails (\τ\̂ significantly \≠ 0 with random T)", "then": "Estimator is overfitting or there is data leakage", "holds": False, "note": "Our placebo estimate = +0.0017 \≈ 0 \✓"},
                    {"if": "Negative control test fails (\τ\̂(T\→NC) significantly \≠ 0)", "then": "Residual confounding exists; adjustment set is incomplete", "holds": False, "note": "Our NC estimate = -0.0008 \≈ 0 \✓"},
                    {"if": "Subset test fails (\τ\̂_80% \≠ \τ\̂_100%)", "then": "Estimate is driven by a small subset; not reliable", "holds": False, "note": "\\u0394 = 0.0052, within tolerance \✓"}
                ],
                "pitfalls": [
                ...
                "refs": "Lipsitch, Tchetgen Tchetgen & Cohen (2010) \“Negative controls: a tool for detecting confounding and bias\” <i>Epidemiology</i>; Shi et al. (2020) negative control exposures; DoWhy (Sharma & Kiciman 2020) refutation framework; Hern\án & Robins (2020) Ch.8 falsification"
            },
        },
        {
            ...
            "thinking": {
                ...
                "steps": [
                    ...
                    {"label": "Mental operation 2: Apply the invariance principle", "detail": "Peters, B\ühlmann & Meinshausen (2016) proved: a correctly specified causal mechanism P(V \\mid parents(V)) is <b>invariant</b> across environments. If the conditional distribution degrades, either the parents are wrong OR the mechanism genuinely changed. This principle bridges causal discovery and causal monitoring."},
